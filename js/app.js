...
if (iconMenu != null) {
	...
	document.querySelector(".icon-block").addEventListener("click", function (e) {
		if (unlock) {
			body_lock(delay);
...
const newsSub = new Swiper('.sub_slider', {
    spaceBetween: 10,
    slidesPerGroup: 1,
    slidesPerView: 3,
    navigation: {
...
const news = new Swiper('.news_slider', {
    thumbs: {
        swiper: newsSub,
    },
    controller: {
...
//
// news.controller.control = newsSub;
// newsSub.controller.control = news;
...
inputFile.forEach(function(el) {
    el.addEventListener('change', function (e) {
        let textSelector = el.parentNode.querySelector('.upload-file__text');
        uploadFile(el.files, textSelector);
    });
});
...
if(openForm != null){
    openForm.forEach((e)=>{
        e.addEventListener('click',()=>{
            ...
            document.querySelectorAll('._vacancy').forEach((e)=>{
                e.classList.remove('_active');
            })
            parent.classList.add('_active')
            let close = parent.querySelector('.close-form');
...
function resize(){
    if (window.innerWidth <= 850){
        let upend = document.querySelector('.uppendBlock');
        let insert = document.querySelector('.insert_block--mobile');
        upend.remove();
        insert.append(upend);
    }
}
window.addEventListener('resize', ()=>{
    if(document.querySelector('.uppendBlock') != null){
        resize();
    }
})
